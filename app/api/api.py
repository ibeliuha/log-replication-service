...
async def post_message(x_token: Annotated[str, Header()],
    ...
    try:
        message_id = await global_entities.SERVICE.register_message(
            message=message,
            api_key=x_token,
            wc=wc
        )
...
@master_router.post('/slaves/register', status_code=200)
def register_to_master(x_token: Annotated[str, Header()], request: Request):
    try:
        service_id = global_entities.SERVICE.register_service(
...
@master_router.get('/slaves', status_code=200)
def get_working_slaves(x_token: Annotated[str, Header()]):
    try:
        registry = global_entities.SERVICE.get_slave_registry(api_key=x_token)
    except AuthorizationError as e:
        raise HTTPException(status_code=e.status_code, detail=str(e))
